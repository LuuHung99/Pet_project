...
const ListTab = [ 
    {
        ...
        text: 'Hashtag drinking vinegar scenester mumblecore snackwave four dollar toast, lumbersexual XOXO. Cardigan church-key pabst, biodiesel vexillologist viral squid.'
    },
    {
        id: 2,
        status: 'bigdrop',
    ...
    },
    {
        ...
        text: 'YOLO drinking vinegar chambray pok pok selfies quinoa kinfolk pitchfork street art la croix unicorn DIY. Woke offal jianbing venmo tote bag, palo santo subway tile slow-carb post-ironic pug ugh taxidermy squid.'
    },
...
function ListTabs(props) {
    ...
    const handleShowTommy = (e) => {
        setShowList('tommy');
    }

    const handleShowBigdrop = () => {
        setShowList('bigdrop');
    }

    const handleShowCuker = () => {
        setShowList('cuker');
    }
    return (
        <>
        {/* {[1,2,3,4].map(e=>{
            ...
            <Row  >
                <Col span={18} push={6}>
                    {data.map((e) => { 
                        if(e.status === showList)
                            return <Tabs items={e} key={e} /> 
                    })}
                </Col>
